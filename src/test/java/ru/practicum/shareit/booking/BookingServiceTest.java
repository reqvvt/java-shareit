...
class BookingServiceTest {
    ...
    void getAllByBookerIdBookingEmpty() {
        ...
        Exception ex = assertThrows(NotFoundException.class, () -> bookingService.getAllByBookerId(booker.getId(),
                "ALL", 0, 3));
        assertEquals("Бронирования отсутствуют.", ex.getMessage());
    }
    ...
    void getAllByBookerIdStatePAST() {
        Booking lastBooking = new Booking(2, LocalDateTime.now().minusDays(3),
    ...
    void getAllByBookerIdStateFUTURE() {
        Booking nextBooking = new Booking(3, LocalDateTime.now().plusDays(1),
    ...
    void getAllByBookerIdStateCURRENT() {
        Booking currentBooking = new Booking(4, LocalDateTime.now().minusDays(1),
                LocalDateTime.now().plusDays(5), item, booker, BookingStatus.APPROVED);
        when(userRepository.findById(anyInt()))
                .thenReturn(Optional.of(booker));
        when(bookingRepository.findAllByBookerId(anyInt(), any(Pageable.class)))
